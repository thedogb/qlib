...
class SignalRecordForMulitScore(RecordTemp):
    """
    ...
    def generate(self, **kwargs):
        ...
        if not isinstance(pred_df, pd.DataFrame):
            raise Exception('SignalRecordForMulitScore 需要模型predict结果产出dataframe')
        pred = pred_df[self.pred_field]
        if isinstance(pred, pd.Series):
            pred = pred.to_frame("score")
        self.save(**{"pred.pkl": pred})
        self.save(**{"pred_df.pkl": pred_df})
